          ...
          <div className={styles.row}>
            ...
            <div className={styles.field}>
              <div className={styles.labelBlock}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button 
                    ...
                    style={{ color: isManualSlug ? 'var(--text-main)' : 'var(--text-muted)' }}
                  >
              ...
              <input 
                ...
                style={{ opacity: isManualSlug ? 1 : 0.6, cursor: isManualSlug ? 'text' : 'not-allowed' }} 
              ...
              {subcategories.map((sub, index) => (
                <div key={index} className={styles.inlineRowItem}>
                  ...
                  <div className={styles.slugInputWrapper} style={{ flex: 1 }}>
                    <input 
                      type="text" 
                      className={`${styles.input} ${styles.slugInputInside}`} 
                      placeholder="Enter subcategory slug" 
                      value={sub.slug} 
                      readOnly={!sub.isManualSlug}
                      onChange={(e) => handleSubcategoryChange(index, 'slug', e.target.value)} 
                      style={{ opacity: sub.isManualSlug ? 1 : 0.6 }}
                    />
                    <button 
                      type="button" 
                      onClick={() => handleSubcategorySlugToggle(index)}
                      className={styles.slugInlineToggle}
                      style={{ color: sub.isManualSlug ? 'var(--text-main)' : 'var(--text-muted)' }}
                    >
